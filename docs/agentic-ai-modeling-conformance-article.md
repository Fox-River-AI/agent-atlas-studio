...
*From battlefield systems to "directionally correct" finance to schema-on-read, the discipline of design kept getting unbundled from the work. Autonomous agents are where that bill finally comes due — and where governance stops being a feature and becomes a product.*

---

Early in my career I built mission-critical systems for the Department of Defense — software whose job was to depict battlefield conditions accurately enough that soldiers made decisions on it. The goal was 100% accuracy. We never quite reached it, and testing always ran long, because no one is capable of architecting and planning for every contingency. But the *standard* was never in question. You designed first. You modeled first. The cost of being wrong was a life, and that cost disciplined everything upstream of the code.

Then I moved into the commercial world, starting in financial services, and I encountered a phrase that has bothered me ever since: **directionally correct.**

My question at the time was simple, and I still don't think it's been answered well: *if you haven't identified the point of failure, how do you know it's directionally accurate?* "Directional" is a claim about error. You cannot make a claim about error you have not characterized. Yet "directionally correct" was rarely a disciplined materiality judgment — it was usually a way to wave off the fact that no one had bounded the error at all. If a report produced a chart the stakeholders felt was approximately right, that was the gold standard.

What I watched over the next two decades was that posture quietly metastasize.
...
I want to be fair here, because the names that get blamed don't deserve it. Ralph Kimball and Bill Inmon are routinely cited as license to stop modeling, and that is a misreading of both. Inmon is the *normalization-first* figure — a centralized, third-normal-form enterprise warehouse as the single source of truth. Kimball's dimensional modeling is denormalized, yes, but it is ferociously disciplined: declared grain, conformed dimensions, explicit handling of slowly changing dimensions, with integrity enforced upstream in the pipeline rather than abandoned. The Kimball-versus-Inmon debate was a fight *between two rigorous modeling schools.* What degraded the field was not their ideas. It was practitioners invoking their authority as cover while doing neither discipline.

The justification was always the same: time to market. And the trade was real — delivery got faster. But quality degraded in lockstep with the architecture that was being skipped, and the degradation was invisible until it wasn't. Outside the DoD, I have rarely seen a software *program* design — no class diagram, no durable structural artifact of any kind. Data models persisted longer than anything else, and that persistence is itself a lesson worth holding onto.

**Data models survived because data outlives code.** A bad schema is brutally expensive to migrate and lives for decades; bad code gets refactored next sprint. Modeling endures exactly where the artifact is durable and costly to change, and withers where it's cheap and disposable. That is why data modeling outlasted UML and class diagrams in commercial practice. Remember that principle — it's about to matter.
...
The symptoms are exactly what the absence of a model always produces. Two agents or MCP servers with identical purposes, implemented differently, because no one could see that the first one already existed. No single canonical location for a capability, so the same logic is reimplemented in three places — and when it needs a patch, no one can say how many places must be corrected. No shared diagram to scan for the obvious structural errors. This is the data-modeling collapse, returning as agent sprawl.

An agent registry is, in effect, the missing Entity Relationship Diagram for the agent estate — the canonical artifact that lets you dedupe, enforce a single source of truth, and *see* the structure before it rots.

But here is where the analogy breaks, and the break is the most important part.
...
It would be easy to read those three pillars and conclude that the dangerous part is the part you *can't* model, and therefore that design-time architecture is the lower-value half — which is roughly the bet the runtime-first camp has made. That conclusion has the dependency exactly backwards. Conformance is the act of comparing running behavior against a *declaration* of intended behavior, and a real declaration is nothing other than design-time architecture. Without Pillar 1, there is no standard for Pillar 3 to measure against; you are left diffing the running system against generic policy, which is how you end up inspecting a house against a building code nobody designed it to. The residual is not an argument against modeling. It is *parasitic* on modeling — the better your architecture defines what the system is supposed to do, the smaller and more legible the residual becomes, and the more meaningful the attestation is. Observability quality is downstream of architectural rigor. You cannot meaningfully watch a residual you never bounded.

Mission-critical systems set the standard. There is no reason less-critical systems can't be designed properly too. The failure of the last twenty years wasn't that we stopped modeling *everything* — a throwaway microsite doesn't merit a formal data model. The failure is that **rigor stopped scaling with the cost of being wrong.** That proportionality collapsed into a single default — *fast, for everything* — and the systems where the cost of failure is highest are now being built with the least architecture of all.

## Why conformance is a product, not a feature

Pillars 1 and 2 are architecture. They are how you build the system that does not fail — or fails only in bounded, identifiable ways. Pillar 3 is the part with no precedent in the data world. And that is exactly why it is a product, and not a feature.

Consider what Erwin actually does that's most valuable. It isn't the diagram. It's Complete Compare: forward-engineer the model into a database, reverse-engineer the database back into a model, and *diff the two* to detect drift. That is declared-versus-running conformance — and it has shipped, for databases, for three decades. So the conformance pattern is not new. The question is why it has always been a *feature* of a modeling tool for data, and why it cannot be for agents.

The answer is in four parts.

**It is continuous, not episodic.** A database drifts only when a human alters it — a column added, a constraint dropped. The running state is stable between those events, so a periodic diff suffices, and a diff you run on a schedule is a button in a tool. An agent drifts *every inference.* There is no stable running state to compare against on a schedule; conformance has to be an always-on process watching a system that is never quiet. An always-on process is not a button. It is an operating system.
